...
const GetBarangayAnnouncement = async (req, res) => {
    ...
    return !result
        ? res.status(400).json({ error: `No such Announcement for Barangay ${brgy}` })
        ...
        if (files) {
            for (let f = 0; f < files.length; f += 1) {
                ...
                if (files[f].originalname === "banner") {
                    ...
                    await deleteFileDrive(announcement.collections.banner[0].id, folder_id);
        ...
        const result = await Announcement.findOneAndUpdate(
            { _id: id },
            {
                $set: {
                    ...
                    collections: {
                        folder_id,
                        banner: banner === null ? announcement.collections.banner[0] : banner,
...
const UpdateAttendees = async (req, res) => {
    try {
        ...
        const result = await Announcement.findByIdAndUpdate({ _id: id }, { $push: { attendees: {firstName, lastName, email, contact, brgy} } },
            { returnOriginal: false, upsert: true })
        return !result
